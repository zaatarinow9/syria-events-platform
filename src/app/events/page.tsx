...
// خريطة الإحداثيات الذكية لربط المحافظة بموقعها على الخريطة
...
export default function EventsPage() {
  ...
  // جلب الفعاليات المنشورة فقط من قاعدة البيانات
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('status', 'published') // الفعاليات المنشورة للعامة فقط
    ...
    <div className="min-h-screen bg-[#F9FAFB] flex flex-col arabic-premium-text" dir="rtl">
      ...
      <section className="container mx-auto px-4 pb-24 flex-1">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-gray-500"><Search className="w-10 h-10 text-gray-300 mb-4" /><p>لا توجد فعاليات مطابقة</p></div>
        )}
      </section>
    </div>
  );
