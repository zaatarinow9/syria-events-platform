...
import { Search, MapPin, Navigation, Calendar, ArrowUpLeft, ShieldAlert, FileText, FileCheck, Send, CheckCircle2, Clock } from "lucide-react";
...
const getEventStatusStyle = (isoDate: string) => {
  const diffDays = Math.ceil((new Date(isoDate).getTime() - new Date().getTime()) / (1000 * 3600 * 24));
  if (diffDays <= 3) return { color: "text-[#ef4444]", bg: "bg-[#ef4444]/10", border: "border-[#ef4444]/20", pulse: "bg-[#ef4444]", label: "قريب جداً" };
  if (diffDays <= 7) return { color: "text-[#f59e0b]", bg: "bg-[#f59e0b]/10", border: "border-[#f59e0b]/20", pulse: "bg-[#f59e0b]", label: "هذا الأسبوع" };
...
export default function Home() {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      const { data } = await supabase.from("permit_requests").select("*").eq("status", "published").order('event_date', { ascending: true });
      if (data) {
  ...
  const filteredEvents = useMemo(() => {
    ...
    return events.filter(e => e.title.toLowerCase().includes(query) || e.location.toLowerCase().includes(query) || e.type.toLowerCase().includes(query));
  }, [events, searchQuery]);
  ...
  const handleGetLocation = () => {
    ...
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        (pos) => { setMapCenter([pos.coords.latitude, pos.coords.longitude]); setMapZoom(12); setIsLocating(false); },
        () => { alert("تعذر تحديد الموقع."); setIsLocating(false); }
      );
    }
  };
          ...
          <div className="w-full lg:w-1/3 flex flex-col h-full space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-4 flex-1 flex flex-col overflow-hidden">
              <div className="space-y-3 mb-4 border-b pb-4">
                ...
                <button onClick={handleGetLocation} className="w-full border py-3 rounded-xl font-bold text-gray-700 flex items-center justify-center gap-2"><Navigation className="w-4 h-4 text-[#073D35]" /> موقعي الحالي</button>
              </div>
