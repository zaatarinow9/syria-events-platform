...
export default function LocationPicker({
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (data && data[0]) {
        ...
        alert(
          "لم نتمكن من العثور على المكان، يرجى سحب الدبوس يدوياً على الخريطة لتحديده."
        );
      }
    } finally {
      setLoading(false);
  ...
  return (
    <div className="space-y-4 w-full flex flex-col">
      ...
      <div
        className={`relative h-[350px] w-full rounded-xl overflow-hidden border-2 border-[#C8A75A]/30 shadow-inner z-0 ${
          readOnly ? "opacity-90" : ""
        }`}
      >
        <MapContainer
        ...
        {!readOnly && (
          <div className="absolute top-4 left-4 z-[1000] bg-white p-3 rounded-xl shadow-lg border border-gray-100 flex items-center gap-2 pointer-events-none">
            ...
            <p className="text-xs text-[#073D35] font-bold">
              انقر أو اسحب الدبوس للموقع الدقيق
            </p>
          </div>
        )}
      </div>
